    ...
    <div class="card-body">
      <form method="post" action="{{route('coupon.update',$coupon->id)}}">
        ...
        <div class="form-group">
          <label for="inputTitle" class="col-form-label">Mã<span class="text-danger">*</span></label>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($coupon->status=='active') ? 'selected' : '')}}>Hiện</option>
            <option value="inactive" {{(($coupon->status=='inactive') ? 'selected' : '')}}>Ẩn</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
